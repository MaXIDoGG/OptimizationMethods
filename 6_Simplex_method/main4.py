...
class SimplexMethod:
    ...
    def get_negative_b_row(self):
        ...
        for i, a_row in enumerate(self.table):
            if a_row[-1] < 0 and (
                row == -1 or abs(a_row[-1]) > abs(self.table[row][-1])
            ):
                row = i
    ...
    def remove_negative_b(self):
        while True:
            row = (
                self.get_negative_b_row()
            )  # ищем строку, в которой находятся отрицательное b
    ...
    def solve(self):
        ...
        while True:
            ...
            if all(
                fi >= 0 if self.mode == MAX_MODE else fi <= 0 for fi in self.f[:-1]
            ):  # если план оптимален
                break  # то завершаем работу

            column = (np.argmin if self.mode == MAX_MODE else np.argmax)(
                self.f[:-1]
            )  # получаем разрешающий столбец
            q = self.get_relations(
                column
            )  # получаем симплекс-отношения для найденного столбца

            if all(
                qi == np.inf for qi in q
            ):  # если не удалось найти разрешающую строку
    ...
    def print_table(self):
        print(
            "     |"
            + "".join(["   y%-3d |" % (i + 1) for i in range(self.variables_count)])
            + "    b   |"
        )

        for i in range(self.restrictions_count):
            print(
                "%4s |" % ("y" + str(self.basis[i] + 1))
        ...
        for row in self.table:
            if full:
                print(
                    " + ".join(
                        [
                            self.print_coef(ai, i)
                            for i, ai in enumerate(row[: self.variables_count])
                            if ai != 0
                        ]
                    ),
                    "=",
                ...
                print(
                    " + ".join(
                        [
                            self.print_coef(ai, i)
                            for i, ai in enumerate(row[: self.main_variables_count])
                            if ai != 0
                        ]
                    ),
                    "<=",
...
def main():
    c = np.array([5, 1])
    a = np.array([[1, 2], [1, 1], [-5, -2], [5, -7], [-1, 1]])

    b = np.array([14, 10, -10, 35, 3])
