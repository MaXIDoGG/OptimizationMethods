...
def argmin(f, x0, S):
    ...
    while abs(b - a) > EPS / 10:
        if f(x0[0] - x1 * S[0], x0[1] - x1 * S[1]) < f(
            x0[0] - x2 * S[0], x0[1] - x2 * S[1]
        ):
            b = x2
        else:
            a = x1
    ...
    while acc > EPS:
        ...
        table.add_row(
            [
                ...
                curX[1],
                f(curX[0], curX[1]),
                abs(f(lastX[0], lastX[1]) - f(curX[0], curX[1])),
            ]
        )
        xPlot.append(curX[0])
        yPlot.append(curX[1])
        ...
        deltX = curX - lastX
        delNu = ((deltX - Nu0 * deltGrad) * np.transpose(deltX - Nu0 * deltGrad)) / (
            np.transpose(deltX - Nu0 * deltGrad) * deltGrad
        )
        Nu0 += delNu
